...
class TLDetector(object):
    ...
    def get_closest_waypoint(self, pose):
        ...
        min_dist = MAX_NUM
        closest_idx = -1
        target_wps = (pose.position.x, pose.position.y)
        dist = lambda a,b: (a[0] - b[0]) ** 2 + (a[1] - b[1]) ** 2
        if self.waypoints:
            for i in range(len(self.waypoints.waypoints)):
                check_wps = (self.waypoints.waypoints[i].pose.pose.position.x, self.waypoints.waypoints[i].pose.pose.position.y)
                check_dist = dist(target_wps, check_wps)
